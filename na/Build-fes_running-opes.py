...
line=f.readline()
mode=line.split()[-1]
if not (mode=='CONVERGE' or mode=='EXPLORE'):
  sys.exit(' unrecognized mode: '+mode)
line=f.readline()
factor=1
if mode=='EXPLORE':
...
def print_fes(it):
  ...
  prob+=epsilon
  z_fes=-factor*kbt*np.log(prob/max(prob))
...
backup='bck.meup.sh '+fes_running_file+file_ext
cmd=subprocess.Popen(backup,shell=True)
cmd.wait()
prob=np.zeros(grid_bin)
...
np.savetxt(fes_running_file+file_ext,np.c_[cv_grid,z_fes],header=head,fmt='%14.9f')

#print final kernels
if print_compressed_kernels:
  filename='compressed-Kernels.data'
